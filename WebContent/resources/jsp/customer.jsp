...
<div id="submenu_customer_list_div_id">
<div class="submenu_bg">
	<s:if test="#session.CUSTOMER_ADD == true">
		<sj:a href="setUpCustomer.action" targets="submenu_customer_list_div_id" indicator="indicatorSubMenuCustomer" cssClass="link"><s:text name="MTIAddCustomer" /></sj:a> |
	</s:if>
	<s:if test="#session.CUSTOMER_VIEW == true">
		<sj:a href="getAllCustomer.action" targets="submenu_customer_list_div_id" indicator="indicatorSubMenuCustomer" cssClass="link"><s:text name="MTIViewCustomer"/></sj:a> |
		<sj:a href="customerSearchForm.action" targets="submenu_customer_list_div_id" indicator="indicatorSubMenuCustomer" cssClass="link"><s:text name="MTISearchCustomer"/></sj:a>
	</s:if>
</div>
<br/>
<img id="indicatorSubMenuCustomer" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_customerList_div_id">
		  <display:table class="tableborder" id="customerListId" name="custList" pagesize="${NO_OF_RECORDS}" requestURI="getAllCustomer.action" sort="list" defaultsort="1" defaultorder="ascending" export="true">
		    <%
		    	try{
		    ...
		    <s:if test="#session.CUSTOMER_VIEW==true">
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_customer_list_div_id','%{listViewCustomer}','');"><s:text name="View"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.CUSTOMER_UPDATE==true">
				<display:column title="${HEdit}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_customer_list_div_id','%{listSetUpCustomer}','');"><s:text name="Edit"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.CUSTOMER_DELETE==true">
				<display:column title="${HDelete}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_customer_list_div_id','%{listDeleteCustomer}','');"><s:text name="Delete"/></s:a>
				</display:column>
			</s:if>
...
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_customerList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>
