...
<div id="submenu_leaveQuota_searchList_div_id">
	<div class="submenu_bg">
		<s:if test="#session.LEAVEQUOTA_ADD == true">
			<sj:a href="setUpEmployeeLeaveQuota.action" targets="submenu_leaveQuota_searchList_div_id" indicator="indicatorSubMenuLeaveQuotaSearchResId_div" cssClass="link"><s:text name="MTIAddLeaveQuota" /></sj:a> |
		</s:if>
		<s:if test="#session.LEAVEQUOTA_VIEW == true">
			<sj:a href="getAllEmployeeLeaveQuota.action" targets="submenu_leaveQuota_searchList_div_id" indicator="indicatorSubMenuLeaveQuotaSearchResId_div" cssClass="link"><s:text name="MTIViewLeaveQuota"/></sj:a> |
			<sj:a href="leaveQuotaSearchForm.action" targets="submenu_leaveQuota_searchList_div_id" indicator="indicatorSubMenuLeaveQuotaSearchResId_div" cssClass="link"><s:text name="MTISearchLeaveQuota"/></sj:a>
		</s:if>
	</div>
	<br/>
	<img id="indicatorSubMenuLeaveQuotaSearchResId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>		   
	    ...
	    <div id="display_tag_leaveQuotaSearchList_div_id">
		    <display:table class="tableborder" id="employeeLeaveQuotaListId" name="empLeaveList" pagesize="${NO_OF_RECORDS}" requestURI="leaveQuotaSearchResult.action" sort="list" defaultsort="1" defaultorder="ascending"  export="true">
		    <%
		    	try{
		    ...
		    %>
		   	<display:column property="empIdObj.empFullName" title="${HELQEmployeeName}" sortable="true" headerClass="sortable"/>
				...
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_leaveQuota_searchList_div_id','%{listViewEmployeeLeaveQuota}','');"><s:text name="View"/></s:a>
				</display:column>
...
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_leaveQuotaSearchList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>  