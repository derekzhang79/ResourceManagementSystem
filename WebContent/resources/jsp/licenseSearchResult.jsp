...
<div id="submenu_license_searchList_div_id">
<div class="submenu_bg">
	<s:if test="#session.LICENSE_ADD == true">
		<sj:a href="setUpLicense.action" targets="submenu_license_searchList_div_id" indicator="indicatorSubMenuLicenseSearchResId_div" cssClass="link"><s:text name="MTIAddLicense" /></sj:a> |
	</s:if>
	<s:if test="#session.LICENSE_VIEW == true">
		<sj:a href="getAllLicense.action" targets="submenu_license_searchList_div_id" indicator="indicatorSubMenuLicenseSearchResId_div" cssClass="link"><s:text name="MTIViewLicense"/></sj:a> |
		<sj:a href="licenseSearchForm.action" targets="submenu_license_searchList_div_id" indicator="indicatorSubMenuLicenseSearchResId_div" cssClass="link"><s:text name="MTISearchLicense"/></sj:a>
	</s:if>
</div>
<br/>
	<img id="indicatorSubMenuLicenseSearchResId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_licenseSearchList_div_id">
		  <display:table class="tableborder" id="licenseListId" name="licenseList" pagesize="${NO_OF_RECORDS}" requestURI="licenseSearchResult.action" sort="list" defaultsort="1" defaultorder="ascending"  export="true">
		    <%
		    	try{
		    ...
		    <s:if test="#session.LICENSE_VIEW==true">
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_license_searchList_div_id','%{listViewLicense}','');"><s:text name="View"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.LICENSE_UPDATE==true">
				<display:column title="${HEdit}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_license_searchList_div_id','%{listSetUpLicense}','');"><s:text name="Edit"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.LICENSE_DELETE==true">
				<display:column title="${HDelete}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_license_searchList_div_id','%{listDeleteLicense}','');"><s:text name="Delete"/></s:a>
				</display:column>
			</s:if>
 ...
 <script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_licenseSearchList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>  