...
<div id="submenu_Holiday_searchList_div_id">
<div class="submenu_bg">
	<s:if test="#session.HOLIDAY_ADD == true">
		<sj:a href="setUpHoliday.action" targets="submenu_Holiday_searchList_div_id" indicator="indicatorSubMenuHolidaySearchResId_div" cssClass="link"><s:text name="MTIAddHoliday" /></sj:a> |
	</s:if>
	<s:if test="#session.HOLIDAY_VIEW == true">
		<sj:a href="getAllHoliday.action" targets="submenu_Holiday_searchList_div_id" indicator="indicatorSubMenuHolidaySearchResId_div" cssClass="link"><s:text name="MTIViewHoliday"/></sj:a> |
		<sj:a href="holidaySearchForm.action" targets="submenu_Holiday_searchList_div_id" indicator="indicatorSubMenuHolidaySearchResId_div" cssClass="link"><s:text name="MTISearchHoliday"/></sj:a>
	</s:if>
</div>
<br/>
<img id="indicatorSubMenuHolidaySearchResId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_holidaySearchList_div_id">
		  <display:table class="tableborder" id="holidayListId" name="holidayList" pagesize="${NO_OF_RECORDS}" requestURI="holidaySearchResult.action" sort="list" defaultsort="1" defaultorder="ascending" export="true">
		    <%
		    	try{
		    ...
		    <s:if test="#session.HOLIDAY_VIEW==true">
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_Holiday_searchList_div_id','%{listViewHoliday}','');"><s:text name="View"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.HOLIDAY_UPDATE==true">
				<display:column title="${HEdit}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_Holiday_searchList_div_id','%{listSetUpHoliday}','');"><s:text name="Edit"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.HOLIDAY_DELETE==true">
				<display:column title="${HDelete}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_Holiday_searchList_div_id','%{listDeleteHoliday}','');"><s:text name="Delete"/></s:a>
				</display:column>
			</s:if>
...
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_holidaySearchList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>   