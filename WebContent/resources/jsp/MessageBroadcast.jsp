...
<div id="submenu_messageBroadcast_list_div_id">
<br/><img id="indicatorSubMenuMessageBroadcastId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_messageBroadcastList_div_id">
		   <display:table class="tableborder" id="messageListId" name="messageList" pagesize="${NO_OF_RECORDS}" requestURI="getAllBroadcastMessage.action" sort="list" defaultsort="1" defaultorder="ascending">
		    <%
		    	try{
		    ...
		    <display:column property="subject" title="${HMessageSubject}" maxLength="20" sortable="true" headerClass="sortable"/>
		     ...
		     <display:column title="${HView}" class="viewedit">
				...
				<s:a href="#" onclick="doPostCall('submenu_messageBroadcast_list_div_id','%{listViewMessage}','');"><s:text name="View"/></s:a>
			</display:column>
	   	    <%-- <display:column title="${HDelete}" class="viewedit">
				<s:url var="listDeleteMessage" action="getAllMyInDeleteBroadcast">
					<s:param name="message.hcmoMessageId" value="#request.MessageId"></s:param>
				</s:url>
				<s:a href="#" onclick="doPostCall('submenu_messageBroadcast_list_div_id','%{listDeleteMessage}','');"><s:text name="Delete"/></s:a>
		    </display:column --%>  
		  </display:table>
	  </div>
</div>  
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_messageBroadcastList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>    