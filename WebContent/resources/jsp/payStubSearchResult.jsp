...
<div id="submen_paystub_searchList_div_id">
	<div class="submenu_bg">
		<s:if test="#session.PAYSTUB_ADD == true">
			<sj:a href="setUpForInsertOrUpdatePayStub.action" targets="submen_paystub_searchList_div_id" indicator="indicatorSubMenuPaystubSearchResId_div" cssClass="link"><s:text name="MTIAddPayStub" /></sj:a> |
		</s:if>
		<s:if test="#session.PAYSTUB_VIEW == true">
			<sj:a href="getAllPayStubs.action" targets="submen_paystub_searchList_div_id" indicator="indicatorSubMenuPaystubSearchResId_div" cssClass="link"><s:text name="MTIViewPayStub"/></sj:a> |
			<sj:a href="payStubSearchForm.action" targets="submen_paystub_searchList_div_id" indicator="indicatorSubMenuPaystubSearchResId_div" cssClass="link"><s:text name="MTISearchPayStub"/></sj:a>
		</s:if>
	</div><br/>
	<img id="indicatorSubMenuPaystubSearchResId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<div class="informationMessageSingle"><li><span><s:text name="label.title.paystub.list"/></span></li></div>	
		...
		<div id="display_tag_payStubSearchList_div_id">
			<display:table class="tableborder" name="payStubList" id="payStubList" pagesize="${NO_OF_RECORDS}" requestURI="payStubSearchResult.action" sort="list" defaultsort="1" defaultorder="ascending"  export="true">
		    	<%
			      try{
		    ...
		    <s:if test="#session.PAYSTUB_VIEW==true">
		    	<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submen_paystub_searchList_div_id','%{getEmployeePayStub}','');"><s:text name="View"/></s:a>
				</display:column>
			 </s:if>
...
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_payStubSearchList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>   