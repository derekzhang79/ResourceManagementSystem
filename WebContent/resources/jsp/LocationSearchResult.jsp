...
<s:form action="skippedPermanently" id="locationSearchSkip">
<div id="submenu_locationSearchList_div_id">
<div class="submenu_bg">
	<s:if test="#session.LOCATION_ADD == true">
		<sj:a href="setUpLocation.action" targets="submenu_locationSearchList_div_id" indicator="indicatorSubMenuLocationSearchResId_div" cssClass="link"><s:text name="MTIAddLocation" /></sj:a> |
	</s:if>
	<s:if test="#session.LOCATION_VIEW == true">
		<sj:a href="getAllLocation.action" targets="submenu_locationSearchList_div_id" indicator="indicatorSubMenuLocationSearchResId_div" cssClass="link"><s:text name="MTIViewLocation"/></sj:a> |
		<sj:a href="locationSearchForm.action" targets="submenu_locationSearchList_div_id" indicator="indicatorSubMenuLocationSearchResId_div" cssClass="link"><s:text name="MTISearchLocation"/></sj:a>
	</s:if>
</div>
<br/>

<img id="indicatorSubMenuLocationSearchResId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="displaytag_locationSearchList_div_id">		   
		  <display:table class="tableborder" id="locationListId" name="location" pagesize="${NO_OF_RECORDS}" requestURI="locationSearchResult.action" sort="list" defaultsort="1" defaultorder="ascending" export="true">
		    <%
		    	try{
		    ...
		    <s:if test="#session.LOCATION_VIEW==true">
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_locationSearchList_div_id','%{listViewLocation}','');"><s:text name="View"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.LOCATION_UPDATE==true">
				<display:column title="${HEdit}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_locationSearchList_div_id','%{listSetUpLocation}','');"><s:text name="Edit"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.LOCATION_DELETE==true">
				<display:column title="${HDelete}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_locationSearchList_div_id','%{listDeleteLocation}','');"><s:text name="Delete"/></s:a>
				</display:column>
			</s:if>
		  ...
		  <table align="center">
				<tr>
					<td>
						...
						<sj:submit href="%{nextConfigurationAction}" value="Next" targets="submenu_locationSearchList_div_id" indicator="indicatorMailConfigurationLocationSearchResId_div" cssClass="submitbutton117"/>
						<img id="indicatorMailConfigurationLocationSearchResId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>
					</td>
					<td>
...
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#displaytag_locationSearchList_div_id").displayTagAjax();
  		}catch(e){
   		}
  	});
</script> 