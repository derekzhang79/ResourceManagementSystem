...
<div id="submenu_leaveApprover_list_div_id">
	<div class="submenu_bg">
		<s:if test="#session.LEAVEAPPROVER_ADD == true">
			<sj:a href="setUpInsertOrUpdateLeaveApprover.action" targets="submenu_leaveApprover_list_div_id" indicator="indicatorSubMenuLeaveApproverId_div" cssClass="link"><s:text name="MTIAddLeaveApprover" /></sj:a> |
		</s:if>
		<s:if test="#session.LEAVEAPPROVER_VIEW == true">
			<sj:a href="getAllLeaveApprover.action" targets="submenu_leaveApprover_list_div_id" indicator="indicatorSubMenuLeaveApproverId_div" cssClass="link"><s:text name="MTIViewLeaveApprover"/></sj:a> |
			<sj:a href="leaveAppSearchForm.action" targets="submenu_leaveApprover_list_div_id" indicator="indicatorSubMenuLeaveApproverId_div" cssClass="link"><s:text name="MTISearchLeaveApprover"/></sj:a>
		</s:if>
	</div>
		<br/>
		<img id="indicatorSubMenuLeaveApproverId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_leaveApproverList_div_id">
		  <display:table class="tableborder" id="leaveApproverListId" name="leaveApproverList" pagesize="${NO_OF_RECORDS}" requestURI="getAllLeaveApprover.action" sort="list" defaultsort="1" defaultorder="ascending"  export="true">
		    <%
		    	try{
		    ...
		    <s:if test="#session.LEAVEAPPROVER_VIEW==true">
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_leaveApprover_list_div_id','%{listViewLeaveApprover}','');"><s:text name="View"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.LEAVEAPPROVER_UPDATE==true">
				<display:column title="${HEdit}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_leaveApprover_list_div_id','%{listSetUpLeaveApprover}','');"><s:text name="Edit"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.LEAVEAPPROVER_DELETE==true">
				<display:column title="${HDelete}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_leaveApprover_list_div_id','%{listDeleteLeaveApprover}','');"><s:text name="Delete"/></s:a>
				</display:column>
			</s:if>
...
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_leaveApproverList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>    