...
<div id="submenu_workExperience_searchList_div_id">
<div class="submenu_bg">
	<s:if test="#session.WORKEXPERIENCE_ADD == true">
		<sj:a href="setUpWorkExperience.action" targets="submenu_workExperience_searchList_div_id" indicator="indicatorSubMenuWorkExperienceSearchResId_div" cssClass="link"><s:text name="MTIAddWorkExperience" /></sj:a> |
	</s:if>
	<s:if test="#session.WORKEXPERIENCE_VIEW == true">
		<sj:a href="getAllWorkExperience.action" targets="submenu_workExperience_searchList_div_id" indicator="indicatorSubMenuWorkExperienceSearchResId_div" cssClass="link"><s:text name="MTIViewWorkExperience"/></sj:a> |
		<sj:a href="workExpSearchForm.action" targets="submenu_workExperience_searchList_div_id" indicator="indicatorSubMenuWorkExperienceSearchResId_div" cssClass="link"><s:text name="MTISearchWorkExperience"/></sj:a>
	</s:if>
</div>
<br/>
	<img id="indicatorSubMenuWorkExperienceSearchResId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_workExperienceSearchList_div_id">
		   <display:table class="tableborder" id="workExperienceListId" name="workexpList" pagesize="${NO_OF_RECORDS}" requestURI="workExpSearchResult.action" sort="list" defaultsort="1" defaultorder="ascending"  export="true">
		    <%
		    	try{
		    ...
		    <s:if test="#session.WORKEXPERIENCE_VIEW==true">
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_workExperience_searchList_div_id','%{listViewWorkExperience}','');"><s:text name="View"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.WORKEXPERIENCE_UPDATE==true">
				<display:column title="${HEdit}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_workExperience_searchList_div_id','%{listSetUpWorkExperience}','');"><s:text name="Edit"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.WORKEXPERIENCE_DELETE==true">
				<display:column title="${HDelete}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_workExperience_searchList_div_id','%{listDeleteWorkExperience}','');"><s:text name="Delete"/></s:a>
				</display:column>
			</s:if>
...
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_workExperienceSearchList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>      