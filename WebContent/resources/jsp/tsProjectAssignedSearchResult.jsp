...
<div id="submenu_tsProjectAssigned_searchList_div_id">
<div class="submenu_bg">
	<sj:a href="setUpTsProjectAssigned.action" targets="submenu_tsProjectAssigned_searchList_div_id" indicator="indicatorSubMenuTsProjectAssignedSearchResId_div" cssClass="link"><s:text name="MTIAddAssignProject" /></sj:a> |
	<sj:a href="getAllTsProjectAssigned.action" targets="submenu_tsProjectAssigned_searchList_div_id" indicator="indicatorSubMenuTsProjectAssignedSearchResId_div" cssClass="link"><s:text name="MTIViewAssignProject"/></sj:a>|
	<sj:a href="tsProjectAssignSearchForm.action" targets="submenu_tsProjectAssigned_searchList_div_id" indicator="indicatorSubMenuTsProjectAssignedSearchResId_div" cssClass="link"><s:text name="MTISearchAssignProject"/></sj:a>
</div>
<br/>
<img id="indicatorSubMenuTsProjectAssignedSearchResId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_tsAssignedSearchList_div_id">
		   <display:table class="tableborder" id="tsProjAssignedListId" name="tsProjAssignedList" pagesize="${NO_OF_RECORDS}" requestURI="tsProjectAssignSearchResult.action" sort="list" defaultsort="1" defaultorder="ascending"  export="true">
		    <%
		    	try{
		    ...
		    <display:column property="projectEndDate" title="${HTsProjAssignedEndDate}" format="{0,date,MM-dd-yyyy}" sortable="true" headerClass="sortable"/>
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_tsProjectAssigned_searchList_div_id','%{listViewTsProjectAssigned}','');"><s:text name="View"/></s:a>
				</display:column>
				<display:column title="${HEdit}" class="viewedit" media="html">
					<s:url var="listsetUpTsProjectAssigned" action="setUpTsProjectAssigned">
						<s:param name="tsProjAssigned.projectAssignEmpId" value="#request.TsProjAssignId"></s:param>
					</s:url>
					<s:a href="#" onclick="doPostCall('submenu_tsProjectAssigned_searchList_div_id','%{listsetUpTsProjectAssigned}','');"><s:text name="Edit"/></s:a>
				</display:column>
				<display:column title="${HDelete}" class="viewedit" media="html">
					<s:url var="listDeleteTsProjectAssigned" action="deleteTsProjectAssigned">
						<s:param name="tsProjAssigned.projectAssignEmpId" value="#request.TsProjAssignId"></s:param>
					</s:url>
					<s:a href="#" onclick="doPostCall('submenu_tsProjectAssigned_searchList_div_id','%{listDeleteTsProjectAssigned}','');"><s:text name="Delete"/></s:a>
				</display:column>
   ...
   <script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_tsAssignedSearchList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>         