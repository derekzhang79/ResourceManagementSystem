...
<div id="submenu_jobTitle_list_div_id">
<div class="submenu_bg">
	<s:if test="#session.JOBTITLE_ADD == true">
		<sj:a href="setUpJobTitle.action" targets="submenu_jobTitle_list_div_id" indicator="indicatorSubMenuJobTitle" cssClass="link"><s:text name="MTIAddJobTitle" /></sj:a> |
	</s:if>
	<s:if test="#session.JOBTITLE_VIEW == true">
		<sj:a href="getAllJobTitle.action" targets="submenu_jobTitle_list_div_id" indicator="indicatorSubMenuJobTitle" cssClass="link"><s:text name="MTIViewJobTitle"/></sj:a> |
		<sj:a href="jobTitleSearchForm.action" targets="submenu_jobTitle_list_div_id" indicator="indicatorSubMenuJobTitle" cssClass="link"><s:text name="MTISearchJobTitle"/></sj:a>
	</s:if>
</div>
<br/>
<img id="indicatorSubMenuJobTitle" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_jobTitleList_div_id">
		  <display:table class="tableborder" id="jobTitleId" name="jobTitleList" pagesize="${NO_OF_RECORDS}" requestURI="getAllJobTitle.action" sort="list" defaultsort="1" defaultorder="ascending" export="true">
		    <%
		    	try{
		    ...
		    <s:if test="#session.JOBTITLE_VIEW==true">
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_jobTitle_list_div_id','%{listViewJobTitle}','');"><s:text name="View"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.JOBTITLE_UPDATE==true">
				<display:column title="${HEdit}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_jobTitle_list_div_id','%{listSetUpJobTitle}','');"><s:text name="Edit"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.JOBTITLE_DELETE==true">
				<display:column title="${HDelete}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_jobTitle_list_div_id','%{listDeleteJobTitle}','');"><s:text name="Delete"/></s:a>
				</display:column>
			</s:if>
		  ...
		  <table align="center">
				<tr>
					<td>
						...
						<sj:submit href="%{nextConfigurationAction}" value="Next" targets="submenu_jobTitle_list_div_id" cssClass="submitbutton117"/>
					</td>
					<td>
						<div class="button-comments">
	   		    		<div class="button-left"></div>
	   		    			<s:url var="skippedPermanently" action="skippedPermanently"/>
							<sj:submit href="%{skippedPermanently}" value="Skipped Permanently" targets="submenu_jobTitle_list_div_id" cssClass="button-midle"/>
						<div class="button-right"></div>
	   		    		</div>
					</td>
					<td>
						<s:url var="skip" action="skip"/>
						<sj:submit href="%{skip}" value="Skip" targets="submenu_jobTitle_list_div_id" cssClass="submitbutton117"/>
					</td>
				</tr>
		</table>
	</s:if> 
...
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_jobTitleList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>  