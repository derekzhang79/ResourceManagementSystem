...
<div id="submenu_SalaryId_div">
	<div class="submenu_bg">
		<s:if test="#session.SALARY_ADD == true">
			<sj:a href="setUpSalary.action" targets="submenu_SalaryId_div" indicator="indicatorSubMenuSalaryId_div" cssClass="link"><s:text name="MTIAddSalary" /></sj:a> |
		</s:if>
		<s:if test="#session.SALARY_VIEW == true">
			<sj:a href="getAllSalary.action" targets="submenu_SalaryId_div" indicator="indicatorSubMenuSalaryId_div" cssClass="link"><s:text name="MTIViewSalary"/></sj:a> |
			<sj:a href="salarySearchForm.action" targets="submenu_SalaryId_div" indicator="indicatorSubMenuSalaryId_div" cssClass="link"><s:text name="MTISearchSalary"/></sj:a>
		</s:if>
	</div>
		<br/>
		<img id="indicatorSubMenuSalaryId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  <s:text name="label.header.salary.salaryName" var="HSalaryName"></s:text>
	  ...
	  <display:table class="tableborder" id="salaryListId" name="salary" pagesize="${NO_OF_RECORDS}" requestURI="getAllSalary.action" sort="list" defaultsort="1" defaultorder="ascending" export="true">
	    <%
	    	try{
	    ...
	    <s:if test="#session.SALARY_VIEW==true">
			<display:column title="${HView}" class="viewedit" media="html">
				...
				<sj:a href="%{listViewSalary}" targets="submenu_SalaryId_div" indicator="indicatorSubMenuSalaryId_div"><s:text name="View"/></sj:a>
			</display:column>
		</s:if>
	    <s:if test="#session.SALARY_UPDATE==true">
			<display:column title="${HEdit}" class="viewedit" media="html">
				...
				<sj:a href="%{listSetUpSalary}" targets="submenu_SalaryId_div" indicator="indicatorSubMenuSalaryId_div"><s:text name="Edit"/></sj:a>
			</display:column>
		</s:if>
	    <s:if test="#session.SALARY_DELETE==true">
			<display:column title="${HDelete}" class="viewedit" media="html">
				...
				<sj:a href="%{listDeleteSalary}" targets="submenu_SalaryId_div" indicator="indicatorSubMenuSalaryId_div"><s:text name="Delete"/></sj:a>
			</display:column>
		</s:if>
