...
<s:form action="skippedPermanently" id="regionSearchSkip">

<div id="submenu_region_searchList_div_id">
<div class="submenu_bg">
	<s:if test="#session.REGION_ADD == true">
		<sj:a href="setUpInsertOrUpdateRegion.action" targets="submenu_region_searchList_div_id" indicator="indicatorSubMenuRegionSearchResId_div" cssClass="link"><s:text name="MTIAddRegion" /></sj:a> |
	</s:if>
	<s:if test="#session.REGION_VIEW == true">
		<sj:a href="getAllRegion.action" targets="submenu_region_searchList_div_id" indicator="indicatorSubMenuRegionSearchResId_div" cssClass="link"><s:text name="MTIViewRegion"/></sj:a> |
		<sj:a href="regionSearchForm.action" targets="submenu_region_searchList_div_id" indicator="indicatorSubMenuRegionSearchResId_div" cssClass="link"><s:text name="MTISearchRegion"/></sj:a>
	</s:if>
</div>
<br/>
<img id="indicatorSubMenuRegionSearchResId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_regionSearchList_div_id">
		  <display:table class="tableborder" id="regionListId" name="region" pagesize="${NO_OF_RECORDS}" requestURI="regionSearchResult.action" sort="list" defaultsort="1" defaultorder="ascending" export="true">
		    <%
		    	try{
		    ...
		    <s:if test="#session.REGION_VIEW==true">
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_region_searchList_div_id','%{listViewRegion}','');"><s:text name="View"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.REGION_UPDATE==true">
				<display:column title="${HEdit}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_region_searchList_div_id','%{listSetUpRegion}','');"><s:text name="Edit"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.REGION_DELETE==true">
				<display:column title="${HDelete}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_region_searchList_div_id','%{listDeleteRegion}','');"><s:text name="Delete"/></s:a>
				</display:column>
			</s:if>
		  ...
		  <table align="center">
				<tr>
					<td>
						...
						<sj:submit href="%{nextConfigurationAction}" value="Next" targets="submenu_region_searchList_div_id" indicator="indicatorMailConfigurationRegionSearchResId_div" cssClass="submitbutton117"/>
						<img id="indicatorMailConfigurationRegionSearchResId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>
					</td>
					<td>
...
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_regionSearchList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>      