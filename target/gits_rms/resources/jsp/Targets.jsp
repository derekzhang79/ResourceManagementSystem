...
<div id="submenu_targets_listId_div">
<div class="submenu_bg">
	<s:if test="#session.TARGETS_ADD == true">
		<sj:a href="setUpTargets.action" targets="submenu_targets_listId_div" indicator="indicatorSubMenuTargetsId_div" cssClass="link"><s:text name="MTIAddTargets" /></sj:a> |
	</s:if>
	<s:if test="#session.TARGETS_VIEW == true">
		<sj:a href="getAllTargets.action" targets="submenu_targets_listId_div" indicator="indicatorSubMenuTargetsId_div" cssClass="link"><s:text name="MTIViewTargets"/></sj:a> |
		<sj:a href="targetsSearchForm.action" targets="submenu_targets_listId_div" indicator="indicatorSubMenuTargetsId_div" cssClass="link"><s:text name="MTISearchTargets"/></sj:a>
	</s:if>
</div>
<br/>
<img id="indicatorSubMenuTargetsId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>				    
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_targetsList_div_id">
		  <display:table class="tableborder" id="targetsListId" name="targetList" pagesize="${NO_OF_RECORDS}" requestURI="getAllTargets.action" sort="list" defaultsort="1" defaultorder="ascending" export="true">
		    <%
		    	try{
		    ...
		    <s:if test="#session.TARGETS_VIEW==true">
				<display:column title="${HView}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_targets_listId_div','%{listViewTargets}','');"><s:text name="View"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.TARGETS_UPDATE==true">
				<display:column title="${HEdit}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_targets_listId_div','%{listSetUpTargets}','');"><s:text name="Edit"/></s:a>
				</display:column>
			</s:if>
		    <s:if test="#session.TARGETS_DELETE==true">
				<display:column title="${HDelete}" class="viewedit" media="html">
					...
					<s:a href="#" onclick="doPostCall('submenu_targets_listId_div','%{listDeleteTargets}','');"><s:text name="Delete"/></s:a>
				</display:column>
			</s:if>
...
<script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_targetsList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>     