...
<div id="submenu_tsAssignedEmpTarget_searchResultList_div_id">
	<div class="submenu_bg">
		<s:if test="#session.TIMESHEET_APPROVER == 'BOTH'">
			<sj:a href="setUpViewTargetGoal.action" targets="submenu_tsAssignedEmpTarget_searchResultList_div_id" indicator="indicatorTsAssignedEmpTargetList" cssClass="link"><s:text name="MTIViewTargets" /></sj:a>
		</s:if>
	</div><br/>
	
	<img id="indicatorTsAssignedEmpTargetList" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>
	...
	<s:set name="NO_OF_RECORDS" value="#session.NO_OF_RECORDS"></s:set>
	  ...
	  <div id="display_tag_tsAssignedEmpTargetSearchList_div_id">
		  <display:table class="tableborder" id="assignTargetListId" name="assignTargetList" pagesize="${NO_OF_RECORDS}" requestURI="getAssignedTargetSearchList.action" sort="list" defaultsort="1" defaultorder="ascending"  export="true">
		  		<%
			    	try{
			    ...
			    <display:column title="${HView}" class="viewedit" media="html">
					<s:url var="getEmployeeAchivedTargetList" action="getEmployeeAchivedTargetList">
						<s:param name="assignTargetObj.hcmoTsAssignProjTargetId" value="#request.empTsId"></s:param>
					</s:url>
					<s:a href="#" onclick="doPostCall('submenu_tsAssignedEmpTarget_searchResultList_div_id','%{getEmployeeAchivedTargetList}','');"><s:text name="View"/></s:a>
				</display:column>
  ...
  <script type="text/javascript">
	//To submit the Sorting and Pagination of DisplayTag in a Div
   	jQuery(document).ready(function() {
   		try{
   		 	jQuery("#display_tag_tsAssignedEmpTargetSearchList_div_id").displayTagAjax();
   		}catch(e){
   		}
  	});
</script>         