...
<%@ taglib prefix="sj" uri="/struts-jquery-tags"%>
	<div id="submenu_Employees_EditForm_div">
	
		<div class="submenu_bg">
			<s:if test="#session.EMPLOYEE_ADD == true">
				<sj:a href="setUpEmployees.action" targets="submenu_Employees_EditForm_div" indicator="indicatorSubMenuEmployeesEditTabId_div" cssClass="link"><s:text name="MTIAddEmployee" /></sj:a> |
			</s:if>
			<s:if test="#session.EMPLOYEE_VIEW == true">
				<sj:a href="getAllEmp.action" targets="submenu_Employees_EditForm_div" indicator="indicatorSubMenuEmployeesEditTabId_div" cssClass="link"><s:text name="MTIViewEmployee"/></sj:a> |
				<sj:a href="employeeSearchForm.action" targets="submenu_Employees_EditForm_div" indicator="indicatorSubMenuEmployeesEditTabId_div" cssClass="link"><s:text name="MTISearchEmployee"/></sj:a>
			</s:if>
		</div>
		<br/>
		<img id="indicatorSubMenuEmployeesEditTabId_div" src="${pageContext.request.contextPath}/resources/images/indicator.gif" alt="Loading..." style="display:none"/>
		<s:if test="#session.USER_NAME==null"><%try{response.sendRedirect("doLogout.action");}catch(Exception e){e.printStackTrace();}%></s:if>
		<s:set name="UniqueEmployeeId" value="employee.employeeId"></s:set>
		
		<s:url var="remoteEmpInformationTab" action="getEmployeeInformationEditTab">
